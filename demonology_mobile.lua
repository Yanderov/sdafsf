...
do
	...
	mkToggle(panels, "Dynamic Island", false, function(v)
		island.Visible = v
...
;(function()
	...
	-- Each entry owns its badge through updateVisuals(); dropping bindKey makes
	-- it hide itself, then the dispatch table is emptied.
	local function stripBinds()
		for key, e in pairs(BindReg) do
			if type(e) == "table" then
				e.bindKey = nil
				if e.updateVisuals then pcall(e.updateVisuals) end
			end
			BindReg[key] = nil
		end
	end
